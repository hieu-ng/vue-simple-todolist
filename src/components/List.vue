			...
			<div class="row">
				<div class="col-md-6 mx-auto">
					...
					<form v-on:submit.prevent="addNewTask">
						...
						<input v-model="taskname" id="tasknameinput" class="form-control"
...
<script>
	...
	export default {
		...
		methods: {
			...
			addNewTask() {
				axios.post('/api/task', { task_name: this.taskname }).then((res) => {
					this.taskname = ''
					this.getTasks()
					console.log(res)
				}).catch((err) => {
			...
			updateTask() {
				axios.put(`/api/task/${this.id}`, { task_name: this.taskname }).then((res) => {
					this.taskname = ''
					this.isEdit = false
					this.getTasks()
					console.log(res)
